...
import { useSupplierPage } from '@/features/suppliers/hooks/useSupplierPage';
import { supplierTranslations, type Language } from '@/i18n/translations';

type SupplierPageContentProps = {
  language: Language;
};

const SupplierPageContent: React.FC<SupplierPageContentProps> = ({ language }) => {
  const t = supplierTranslations[language];
  const {
    suppliers,
    editingId,
    searchTerm,
    ...
    <div className="flex gap-4 items-start">
      <div className="w-[30%] shrink-0 bg-white shadow-sm border border-gray-200 p-4">
        <h4 className="text-base font-medium text-gray-900 mb-4">
          {editingId ? t.editSupplier : t.createSupplier}
        </h4>
        <form onSubmit={onSubmit} className="space-y-3">
          <label className="block">
            ...
            <input
              ...
              onChange={(e) => setSupplierInput((prev) => ({ ...prev, name: e.target.value }))}
          ...
          <label className="block">
            <span className="text-sm font-medium text-gray-700">{t.contactPerson}</span>
            <input
              type="text"
              value={supplierInput.contactPerson}
              onChange={(e) => setSupplierInput((prev) => ({ ...prev, contactPerson: e.target.value }))}
              className="mt-1 block w-full border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500"
            />
          </label>
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(e) => setSupplierInput((prev) => ({ ...prev, email: e.target.value }))}
              className="mt-1 block w-full border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500"
            />
          </label>
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(e) => setSupplierInput((prev) => ({ ...prev, phone: e.target.value }))}
              className="mt-1 block w-full border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500"
            />
          </label>
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(e) => setSupplierInput((prev) => ({ ...prev, address: e.target.value }))}
              className="mt-1 block w-full border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500"
            />
          </label>
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(e) => setSupplierInput((prev) => ({ ...prev, city: e.target.value }))}
              className="mt-1 block w-full border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500"
            />
          </label>
          ...
          <div className="flex gap-2 pt-1">
            <button type="submit" className="px-4 py-2 text-sm font-semibold text-white bg-green-600 hover:bg-green-700">
              {editingId ? 'Save Changes' : t.createSupplier}
            </button>
            {editingId && (
              <button
                type="button"
      ...
      <div className="w-[70%] min-w-0 bg-white shadow-sm border border-gray-200 p-3">
        <div className="flex items-center gap-2 mb-3">
          <h3 className="text-lg font-semibold text-gray-900">{t.suppliers}</h3>
          <p className="text-xs text-gray-500">({suppliers.length} {t.supplier}{suppliers.length !== 1 ? 's' : ''})</p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t.contactPerson}</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredSuppliers.map((supplier) => (
                <tr key={supplier.id} className={editingId === supplier.id ? 'bg-indigo-50' : ''}>
                  <td className="px-3 py-2 text-sm text-gray-700">{supplier.name}</td>
                  <td className="px-3 py-2 text-sm text-gray-700">{supplier.contactPerson}</td>
                  <td className="px-3 py-2 text-sm text-gray-700">{supplier.email}</td>
                  <td className="px-3 py-2 text-sm text-gray-700">{supplier.phone}</td>
                  <td className="px-3 py-2 text-sm text-gray-700">{supplier.city}</td>
              ...
              {filteredSuppliers.length === 0 && (
                <tr>
                  <td className="px-3 py-4 text-sm text-gray-500" colSpan={6}>{t.noSuppliersFound}</td>
                </tr>
              )}
            </tbody>
          </table>
